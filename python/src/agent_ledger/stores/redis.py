...
from agent_ledger.models import CommitReceipt, EventStream, ProposedEvent, StoredEvent, utc_now
...
local receipt = {
    stream = {session_id = ARGV[6], stream_id = ARGV[7]},
    append_id = ARGV[3],
    digest = ARGV[4],
...
class RedisEventStore:
    ...
    async def append(
        ...
        args: list[str | int] = [
            expected_version,
            f"version:{_hash(stream.stream_id)}",
            append_id,
            digest,
    ...
    async def scan_session(
        ...
        stream = EventStream(session_id=session_id, stream_id="_")
        keys = self._keys(stream)
        try:
            event_ids = await asyncio.wait_for(
    ...
    def _keys(self, stream: EventStream) -> list[str]:
        tag = _hash(stream.session_id)
        root = f"{self._prefix}:{{{tag}}}"
        stream_key = _hash(stream.stream_id)
        return [
            f"{root}:meta",
            f"{root}:events",
            f"{root}:session-index",
            f"{root}:stream-index:{stream_key}",
            f"{root}:commits:{stream_key}",
        ]
