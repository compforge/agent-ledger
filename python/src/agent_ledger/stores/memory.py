...
class MemoryEventStore:
    ...
    async def append(
        ...
        receipt_key = (stream.session_id, stream.stream_id, append_id)
        stream_key = (stream.session_id, stream.stream_id)
        ...
        async with self._lock:
            ...
            for event in batch:
                if (stream.session_id, event.event_id) in self._event_ids:
            ...
            self._event_ids.update((stream.session_id, event.event_id) for event in stored)
    ...
    async def read_stream(
        ...
        async with self._lock:
            events = [
                ...
                for event in self._stream_events.get((stream.session_id, stream.stream_id), [])
