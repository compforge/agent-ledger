...
class _EventStream(_Base):
    __tablename__ = "agent_ledger_streams"

    session_id: Mapped[str] = mapped_column(String(_ID_LENGTH), primary_key=True)
    stream_id: Mapped[str] = mapped_column(String(_ID_LENGTH), primary_key=True)
...
class _Event(_Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "session_id",
            "event_id",
            name="uq_agent_ledger_session_event",
        ),
    ...
    event_id: Mapped[str] = mapped_column(String(_ID_LENGTH), nullable=False)
    session_id: Mapped[str] = mapped_column(String(_ID_LENGTH), nullable=False)
    stream_id: Mapped[str] = mapped_column(String(_ID_LENGTH), nullable=False)
                ...
                async with self._engine.begin() as connection:
                    ...
                    if version is None:
                        await connection.execute(
                            insert(_SchemaVersion).values(component="core", version=2)
                        )
                    elif version != 2:
            ...
            async with session.begin():
                existing = await session.scalar(
                    ...
                    .where(
                        ...
                        _AppendCommit.stream_id == stream.stream_id,
                ...
                stream_row = await session.scalar(
                    select(_EventStream)
                    .where(
                        _EventStream.session_id == stream.session_id,
                        _EventStream.stream_id == stream.stream_id,
                    )
                    .with_for_update()
                )
                if stream_row is None:
                    stream_row = _EventStream(
            ...
            async with asyncio.timeout(self._operation_timeout):
                async with AsyncSession(self._engine) as session:
                    result = await session.scalars(
                        ...
                        .where(
                            _Event.session_id == stream.session_id,
                            _Event.stream_id == stream.stream_id,
