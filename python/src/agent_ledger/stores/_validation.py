...
from agent_ledger.models import EventStream, ProposedEvent, canonical_append_digest
...
def validate_append(
    ...
    for event in batch:
        if event.session_id != stream.session_id:
            raise StoreError("all events must belong to the target stream's session")
