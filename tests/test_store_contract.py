...
async def test_atomic_batch_versions_and_reads(event_store: EventStore) -> None:
    ...
    stream = RunStream(session_id=session_id, run_id=str(uuid4()))
    first = event(session_id, stream.run_id)
    second = event(session_id, stream.run_id)
...
async def test_append_is_idempotent_by_content(event_store: EventStore) -> None:
    session_id = str(uuid4())
    stream = RunStream(session_id=session_id, run_id=str(uuid4()))
    item = event(session_id, stream.run_id)
    append_id = str(uuid4())
    ...
    with pytest.raises(IdempotencyViolation):
        await event_store.append(stream, -1, append_id, [event(session_id, stream.run_id)])


async def test_optimistic_concurrency(event_store: EventStore) -> None:
    session_id = str(uuid4())
    stream = RunStream(session_id=session_id, run_id=str(uuid4()))
    await event_store.append(stream, -1, str(uuid4()), [event(session_id, stream.run_id)])
...
async def test_duplicate_event_rolls_back_whole_batch(event_store: EventStore) -> None:
    session_id = str(uuid4())
    first_stream = RunStream(session_id=session_id, run_id=str(uuid4()))
    duplicate_id = str(uuid4())
    await event_store.append(
        first_stream,
        -1,
        str(uuid4()),
        [event(session_id, first_stream.run_id, event_id=duplicate_id)],
    )

    second_stream = RunStream(session_id=session_id, run_id=str(uuid4()))
    with pytest.raises(DuplicateEvent):
        await event_store.append(
            ...
            str(uuid4()),
            [
                event(session_id, second_stream.run_id),
                event(session_id, second_stream.run_id, event_id=duplicate_id),
            ],
        )
...
async def test_session_scan_merges_runs_and_pages(event_store: EventStore) -> None:
    ...
    first_stream = RunStream(session_id=session_id, run_id=str(uuid4()))
    second_stream = RunStream(session_id=session_id, run_id=str(uuid4()))
    first = event(session_id, first_stream.run_id)
    second = event(session_id, second_stream.run_id)
    third = event(session_id, first_stream.run_id)
